...
def conv2d(input_, output_dim, d_h=2, d_w=2, scope='conv_0',
    ...
    k_initializer = tf.compat.v1.random_normal_initializer(stddev=0.02)
    b_initializer = tf.compat.v1.constant_initializer(0)
...
def content_loss(endpoints_mixed, content_layers):
    ...
    for layer in content_layers:
        ...
        size = tf.size(feat_a)
        loss += tf.nn.l2_loss(feat_a - feat_b) * 2 / tf.cast(size, dtype=tf.float32)
...
def style_loss(endpoints_mixed, style_layers):
    ...
    for layer in style_layers:
        ...
        size = tf.size(feat_a)
        loss += tf.nn.l2_loss(
            gram(feat_a) - gram(feat_b)) * 2 / tf.cast(size, dtype=tf.float32)
...
def gram(layer):
    ...
    denominator = tf.cast(width * height * num_filters, dtype=tf.float32)
